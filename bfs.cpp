...
//    return 0;
    ...
    for(int i = 0 ; i< edge ; i++)
    {
        //cout << "Enter 2 Nodes : ";
        cin >> node1 >> node2;
        adj[node1][node2] = 1; //giving edge number
        adj[node2][node1] = 1;

    }
    for(int i = 0 ; i < vertex ; i++) //checking all adjacent are OK
    {
...
void bfs(int adj[][SIZE] , int vertex , int startingNode)
{
    for(int i = 0 ; i < vertex ; i++)
    {
        if(i!= startingNode)
        {
            colour[i] = WHITE; //declared on TOP
    ...
    queue <int> bfsQueue; //empty queue declare
    bfsQueue.push(startingNode);
    cout << "BFS is : ";
    while(!bfsQueue.empty()) //if queue is not empty 
    {
        int u = bfsQueue.front(); //push front of bfsQ
        cout << u << " ";
        bfsQueue.pop(); //front element pop
        for(int i = 0 ; i < vertex ; i++)
        {
            if(adj[u][i]!=0) //not equal means has connection
            {
                int v = i;
                if(colour[v] == WHITE)
                {
